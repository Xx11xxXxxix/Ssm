...
export default {
	...
	methods: {
		getCartList: function() {
			...
			util.request(api.CartList).then(function(res) {
				if (res.errno === 0) {
					...
					console.log("购物车数据更新成功:", res.data);
				}else {
                    console.log("获取购物车列表失败:", res.errmsg);
				}
				that.checkedAllStatus = that.isCheckedAll()
				console.log("全选状态更新:", that.checkedAllStatus);
			});
		},
		isCheckedAll: function() {
			//判断购物车商品已全选
			console.log("检查是否全部选中:", allChecked);
		...
		checkedItem: function(event) {
			...
			let that = this;
			let isChecked = that.cartGoods[itemIndex].checked ? 0 : 1;
			console.log(`更改商品选择状态, 商品索引: ${itemIndex}, 状态: ${isChecked}`);
			if (!that.isEditCart) {
				util.request(api.CartChecked, {
					...
					if (res.errno === 0) {
						that.cartGoods = res.data.cartList
						that.cartTotal = res.data.cartTotal
						console.log("商品选中状态更新成功:", res.data);
					}
		...
		checkedAll: function() {
			let that = this;
			let isChecked = this.isCheckedAll() ? 0 : 1;
            console.log("全选或取消全选:", isChecked);
		...
		checkoutOrder: function() {
			...
			var checkedGoods = that.cartGoods.filter(function(element, index, array) {
				console.log("结账商品:", checkedGoods);
				if (element.checked == true) {
					return true;
				} else {
					console.log("无选中商品进行结账");
					return false;
				}
			});
